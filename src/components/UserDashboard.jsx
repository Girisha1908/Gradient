...
const sideNav = [
  { icon: 'assignment_turned_in', label: 'My Tasks' }, { icon: 'group', label: 'Team' },
  { icon: 'insights', label: 'Analytics' }, { icon: 'settings', label: 'Settings' },
];
...
const UserDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const { data: l } = await supabase.from('activity_logs').select('*').eq('user_id', realUserId).order('created_at', { ascending: false }).limit(5);
      setTimeline(l || []);
    } catch (e) {
    ...
    <div className="bg-[#fafafa] text-[#1d1d1d] min-h-screen" style={{ fontFamily: "'Geist Sans', sans-serif" }}>
      ...
      <aside className="fixed left-0 top-0 h-full w-72 bg-white/40 backdrop-blur-xl border-r border-[#f4f4f5] flex flex-col py-10 px-8 z-50">
        ...
        <nav className="flex-1 space-y-2">
          <a className="flex items-center gap-4 py-3.5 px-5 bg-white rounded-2xl border border-[#f4f4f5] shadow-sm" href="#">
            <span className="material-symbols-outlined text-[20px]">grid_view</span>
            <span className="text-[13px] font-semibold tracking-tight">Dashboard</span>
          </a>
          {sideNav.map(i => (
            <a key={i.label} className="flex items-center gap-4 py-3.5 px-5 text-[#71717a] hover:text-[#1d1d1d] transition-all group" href="#">
            ...
            <div className="lg:col-span-2 space-y-12">
              <section className="bg-white p-12 rounded-[3rem] border border-[#f4f4f5] shadow-[0px_20px_50px_rgba(0,0,0,0.03)]">
                ...
                <div className="space-y-6">
                  {tasks.length === 0 ? <p className="text-sm text-[#71717a] py-10 text-center italic opacity-60">Your workspace is quiet. No active assignments found.</p> : tasks.map(t => {
                    const status = getTaskStatus(t);
                    return (
                    <div key={t.id} className="group p-6 rounded-2xl border border-black/[0.03] bg-white/50 hover:bg-white hover:border-black transition-all flex items-center justify-between shadow-sm hover:shadow-md">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col h-[600px] border border-black/5 animate-fade-in">
            ...
            <div className="p-4 border-t border-black/5 bg-white flex gap-3 z-10">
              ...
              />
              <button onClick={handleSend} disabled={!newMessage.trim()} type="button" className="w-12 h-12 flex-shrink-0 bg-black text-white rounded-full flex items-center justify-center disabled:opacity-30 hover:opacity-80 transition-opacity shadow-md">
